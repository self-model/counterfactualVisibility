function model_predictions = get_model_predictions_softmax(true_thetas, believed_thetas, gamma, ndt, softmax, dt, T)
    ...
    for t = T*2:-1:0
        ...
        A(:,t+1,states+1)=exp(V_by_A(:,t+1,states+1)*softmax);
    ...
    ndt_indices = [floor(ndt/dt), ceil((ndt)/dt)]; % Convert NDT from seconds to time points
    ndt_weights = [1-mod(ndt/dt,1), mod(ndt/dt,1)];
