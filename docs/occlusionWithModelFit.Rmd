...
(ref:main-results) Main results from Experiments 1-3. A) miss and false alarm rates as a function of occlusion level. B) mean median reaction times in target-present and target-absent correct responses, as a function of occlusion level. C) mean confidence in target-present and target-absent correct responses, as a function of occlusion level. Error bars represent the standard error. \*: p\<0.05, \*\*: p\<0.01, \*\*\*: p\<0.001

```{r main-results, echo=FALSE, fig.cap="(ref:main-results)"}
knitr::include_graphics("figures/results_one_panel.png")
```
...
## Additional data collection: revealing individual differences in occlusion effects on inference about absene
...
This population-mixture model predicts that despite a group-level null effect, individual participants should show reliable effects of occlusion on “target absent” reaction times: negative for some participants, and positive for others. To test this prediction, we collected a large number of test trials from a subset of participants who took part in Exp. 2 and 3 (see Fig. \@ref(fig:sc-results)B). We contacted all participants who had accuracy of 70% or higher, and who did not require more than one iteration over the instructions before passing the comprehension check. The first 10 participants from each study to accept our invitation were invited to take part in 5 20-minute experiments, which they could complete in their own free time. Over the course of these sessions we collected 896 trials per participant, with the exception of two participants in Exp. 3 for which we have 672 and 864 trials.
...
#             "E3a.ta.sign_consistency"), file="sc.Rdata")
...
In order to map this behavioural variability onto the model parameter space, we fitted model parameters to the response and response time data of participants (details about the model fit procedure are provided in the Methods). Model parameters are listed in table 1. Of note, model fits were free to independently vary in the physical visibility of stimuli (parameters $\theta$ and $\alpha$ ) and in participants’ expectations regarding these quantities (parameters $\bar\theta$ and $\bar\alpha$).

<!-- As shown in Fig. X, the model captures key aspects in the data, including the pattern of errors and response times in decisions about presence and absence, and the group-level effect of occlusion. Note that the model captures the overall bias to report absence (X of all responses) although the prior probability of target absence is assumed to be known to be 0.5, and the incentive structure is fully symmetric with respect to false alarms and misses. A response bias emerges due to asymmetries in the likelihood function, and the different informational value of evidence for presence versus absence.  -->

As shown in Fig. \@ref(fig:fit)A and E, the model captures key aspects in the data, including the pattern of errors and response times in decisions about presence and absence, and the group-level effect of occlusion. The presented model fails to account for the difference in response times between correct and incorrect responses, but a model that assumes variability in trial difficulty successfully captures this aspect of the data (see Appendix). Note that the model captures the overall bias to report absence in Exp. 2 (`r E2.sim.df%>%mutate(resp=ifelse(present=='present',correct,1-correct))%>%group_by(subj_id)%>%summarise(bias=mean(resp))%>%pull(bias)%>%mean()%>%printnum()` of all observed and simulated responses) and the overall bias to report presence in Exp. 3 (`r E2.sim.df%>%mutate(resp=ifelse(present=='present',correct,1-correct))%>%group_by(subj_id)%>%summarise(bias=mean(resp))%>%pull(bias)%>%mean()%>%printnum()` of all simulated responses, `r E2.minimal_df%>%mutate(resp=ifelse(present=='present',correct,1-correct))%>%group_by(subj_id)%>%summarise(bias=mean(resp))%>%pull(bias)%>%mean()%>%printnum()` of all observed responses) although the prior probability of target absence is assumed to be known to be 0.5 in both experiments, and the incentive structure is fully symmetric with respect to false alarms and misses. A response bias emerges due to asymmetries in the likelihood function, and the different informational value of evidence for presence versus absence.

Inspecting the fitted model parameters revealed that overall, $alpha$ and $\bar\alpha$ were correlated across individuals (Exp. 1: `r apa_print(cor.test(E1_parameters$alpha,E1_parameters$belalpha))$estimate`; Exp. 2: `r apa_print(cor.test(E2_parameters$alpha,E2_parameters$belalpha))$estimate`; Exp. 3: `r apa_print(cor.test(E3_parameters$alpha,E3_parameters$belalpha))$estimate`), meaning participants' beliefs about the effects of occlusion on visibility were proportional to the true effect occlusion had on stimulus visibility. Importantly, despite this strong alignment, participants had an overall tendency to act in accordance with a belief that occlusion affected visibility to a lesser degree than its true effect (E1: `r apa_print(t.test(E1_parameters$belalpha-E1_parameters$alpha))$statistic`; E2: `r apa_print(t.test(E2_parameters$belalpha-E2_parameters$alpha))$statistic`; E3: `r apa_print(t.test(E3_parameters$belalpha-E3_parameters$alpha))$statistic`). While occlusion had a similar effect on $\alpha$ in Exp. 2 and 3 (`r apa_print(t.test(E2_parameters$alpha,E3_parameters$alpha))$statistic`), the added reference stimuli in Exp. 3 affected $\bar\alpha$, bringing it closer to $alpha$ itself (a contrast between $|\alpha-\bar\alpha|$ in Exp. 2 and 3: `r apa_print(t.test(abs(E2_parameters$alpha-E2_parameters$belalpha),abs(E3_parameters$alpha-E3_parameters$belalpha)))$statistic`). This is in line with the theoretical interpretation of these two model parameters as being based in the true and believed effects of occlusion on visibility, respectively.
...
plot_rt_correlation <- function(human_df, sim_df, fi_df, ni_df, exp_number) {
  ...
  rta_df <- human_df %>% 
    ...
    group_by(subj_id) %>%
    summarise(human = mean(rt[occlusion_is_low==0])-mean(rt[occlusion_is_low==1])) %>%
    merge(sim_df %>% 
    filter(present=='absent' & correct==1) %>%
    group_by(subj_id) %>%
    summarise(sim = mean(rt[occlusion_is_low==0])-mean(rt[occlusion_is_low==1])), by='subj_id') %>%
    merge(fi_df %>% 
    filter(present=='absent' & correct==1) %>%
    group_by(subj_id) %>%
    summarise(fi = mean(rt[occlusion_is_low==0])-mean(rt[occlusion_is_low==1])), by='subj_id')%>%
    merge(ni_df %>% 
    filter(present=='absent' & correct==1) %>%
    group_by(subj_id) %>%
    summarise(ni = mean(rt[occlusion_is_low==0])-mean(rt[occlusion_is_low==1])), by='subj_id')
  ...
  rta_df %>% 
    ...
    geom_point(color=detection_colors[2], alpha=0.2) +
    theme_classic() +
    ...
    geom_smooth(method='lm', alpha=0.3, color=detection_colors[2], fill=detection_colors[2])+
    geom_smooth(aes(x=human,y=fi),method='lm', alpha=0.3, color='black', fill='black') +
    geom_smooth(aes(x=human,y=ni),method='lm', alpha=0.3, color='brown', fill='brown') 
  ...
  rtp_df %>% 
    ...
    geom_hline(yintercept=0, alpha=0.15)+
    geom_vline(xintercept=0, alpha=0.15)+
    geom_point(color=detection_colors[1], alpha=0.2) +
    theme_classic() +
    theme(aspect.ratio=1)+
    coord_fixed()+
    ...
    geom_smooth(method='lm', alpha=0.3, color=detection_colors[1], fill=detection_colors[1])+
    geom_smooth(aes(x=human,y=fi),method='lm', alpha=0.3, color='black', fill='black')+
    geom_smooth(aes(x=human,y=ni),method='lm', alpha=0.3, color='brown', fill='brown') 
...
plot_error_correlation <- function(human_df, sim_df, fi_df, ni_df, exp_number) {
  ...
  errora_df %>% 
    ...
    labs(x='observed false alarm effect',
         y='recovered false alarm effect')+
    geom_smooth(method='lm', alpha=0.3, color=detection_colors[2], fill=detection_colors[2])+
    geom_smooth(aes(x=human,y=fi),method='lm', alpha=0.3, color='black', fill='black')+
    geom_smooth(aes(x=human,y=ni),method='lm', alpha=0.3, color='brown', fill='brown') 
  ...
  errorp_df %>% 
    ...
    theme_classic() +
    theme(aspect.ratio=1)+
    scale_x_continuous(limits = c(floor(min(errorp_df$human,errorp_df$sim)*5)/5,ceil(max(errorp_df$human,errorp_df$sim)*5)/5))+
    scale_y_continuous(limits = c(floor(min(errorp_df$human,errorp_df$sim)*5)/5,ceil(max(errorp_df$human,errorp_df$sim)*5)/5))+
    coord_fixed()+
    labs(x='observed miss effect',
         y='recovered miss effect')+
    geom_smooth(method='lm', alpha=0.3, color=detection_colors[1], fill=detection_colors[1])+
    geom_smooth(aes(x=human,y=fi),method='lm', alpha=0.3, color='black', fill='black')+
    geom_smooth(aes(x=human,y=ni),method='lm', alpha=0.3, color='brown', fill='brown') 
  
  ggsave(paste('../docs/figures/MLE_parameter_simulations/basic_model/errors/recovery/effect_recovery_miss',exp_number,'.png',sep=''), width=3.2,height=3.2, dpi=600)
...
To test the idea that variability in the effects of occlusion on perceptual decisions reflects more than variability in the physical effects of occlusion on visibility, we fitted two reduced models, using the same model-fitting procedure. The first reduced model forced the constraint that $\alpha=\bar\alpha$, effectively forcing a $V_{INCORP}$-like behaviour. The second reduced model forced the constraint that $\bar\alpha=1$, rendering participants unaware of the effects of occlusion on visibility, similar to $V_{IGNORE}$. We then measured how well the two models captured population variability in the effects of occlusion by taking the Pearson correlation between observed and simulated effects, across participatns. A much smaller proportion of variability in target-absent response times could be accounted for by $V_{IGNORE}$ than by the full model (`r apa_print(cor.test(E1.rt_correlations%>%filter(present==0)%>%pull(human),E1.rt_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E1.rt_correlations%>%filter(present==0)%>%pull(human),E1.rt_correlations%>%filter(present==0)%>%pull(ni)))$estimate`, $p=0.002$; Exp. 2: :`r apa_print(cor.test(E2.rt_correlations%>%filter(present==0)%>%pull(human),E2.rt_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E2.rt_correlations%>%filter(present==0)%>%pull(human),E2.rt_correlations%>%filter(present==0)%>%pull(ni)))$estimate`, $p<0.001$; Exp. 3: :`r apa_print(cor.test(E3.rt_correlations%>%filter(present==0)%>%pull(human),E3.rt_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E3.rt_correlations%>%filter(present==0)%>%pull(human),E3.rt_correlations%>%filter(present==0)%>%pull(ni)))$estimate`, $p<0.001$; correlations were compared using Steiger's test for dependent correlations, @steiger1980 ). Similarly, a much smaller proportion of variability in the effect of occlusion on false alarms could be explained by $V_{incorp}$ than by the full model (Exp. 1:`r apa_print(cor.test(E1.error_correlations%>%filter(present==0)%>%pull(human),E1.error_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E1.error_correlations%>%filter(present==0)%>%pull(human),E1.error_correlations%>%filter(present==0)%>%pull(fi)))$estimate`, $p<0.001$; Exp. 2: `r apa_print(cor.test(E2.error_correlations%>%filter(present==0)%>%pull(human),E2.error_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E2.error_correlations%>%filter(present==0)%>%pull(human),E2.error_correlations%>%filter(present==0)%>%pull(fi)))$estimate`, $p<0.001$; Exp. 3: `r apa_print(cor.test(E3.error_correlations%>%filter(present==0)%>%pull(human),E3.error_correlations%>%filter(present==0)%>%pull(sim)))$estimate` versus `r apa_print(cor.test(E3.error_correlations%>%filter(present==0)%>%pull(human),E3.error_correlations%>%filter(present==0)%>%pull(fi)))$estimate`, $p<0.001$). Together, the effect of occlusion on participants' error rates and response times could not be fully captured by single perceptual parameter. An additional parameter, representing participants' beliefs about the effects of occlusion on target visibility, was required to account for population heterogeneity.
...
rt_df <- E1.minimal_df %>% 
  ...
  merge(params_df %>%
          dplyr::select(exp,subj_id,alpha,belalpha,gamma), by=c('exp','subj_id')) 
...
Finally, we compared the unique effects of $\alpha$ (true visibility) and $\bar\alpha$ (beliefs about visibility) on accuracy and reaction times in target-present and target-absent trials. This was done by fitting a multiple regression model, predicting individual-level contrasts of interest (for example, the standardised average reaction times in high versus low occlusion, extracted for individual subjects) from $\alpha$ and $\bar\alpha$, and comparing the explained variance in the full model with the explained variance in the reduced model, dropping the parameter of interest. A clear picture appears: while $\alpha$ contributes more than $\bar\alpha$ to decision accuracy and decision times in target-present trials, the opposite is true for target-absent trials. Decisions about the absence of stimuli are affected by participants' beliefs about the effects of occlusion on visiblity much more than its actual effects on visibility. 
...
Finally, "target absent" decision times uncover robust heterogeneity in the incorporation of beliefs about counterfactual visibility into perceptual decisions. This heterogeneity further underscores the complexity of the cognitive processes involved in deceptively simple perceptual decisions.
...
Our computational model provides an illustration for a setting in which a rational agent would show qualitatively similar behavioural patterns to the ones we observe in our participants. This way, we were able to explain the slower decision times and lower confidence ratings in decisions about absence, the effect of stimulus occlusion on hit rate, and the variable effects of occlusion on the accuracy, speed and confidence of decisions about presence and absence. We do not show, however, a quantitative fit to the behaviour of single individuals. The reason is that doing so would be computationally taxing: for every set of $\alpha$, $\bar\alpha$, $\theta_{present}$, $theta_{absent}$ and $\gamma$, we would need to derive the optimal policy before deriving the expected behavioural patterns analytically or numerically.

Relatedly, while our implementation of model variants $A$ and $B$ provides good qualitative predictions for the direction of effects in the two groups, it also makes the wrong prediction that a 50-50 population mixture of the two variants should result in a negative effect of occlusion on the false alarm rate, and not a positive one as we observe (compare the slopes of the red lines in the top panel of Fig. \@ref(fig:model-occlusion)C). Alternatively, assuming variant $A$ is more prevalent would result in a net positive effect of occlusion on target-absent reaction times, again inconsistent with the observed null effect. Our current model may thus be incomplete in representing the quantitative relationship between error rates and reaction times, or in representing how the two are affected by expected visibility. Alternatively, a more systematic model-fitting may identify parameter combinations for which predictions about population-level reaction time and accuracy effects simultaneously hold.

Finally, our psychological interpretation of model variant $B$ was of a failure to incorporate counterfactuals into perceptual decisions. However, an equally valid interpretation is one of incomplete, or entirely missing, meta-perceptual knowledge about the effects of occlusion on perception. Indeed, knowing that occlusion affects stimulus visibility is a precondition for rationally incorporating this knowledge into the decision-making process. While this may seem like an unlikely account, an increase in the effect of occlusion on the false alarm rate between Exp. 2 and 3 indicates that having direct access to the effects of occlusion on stimulus visibility does facilitate the use of counterfactuals in perception. Importantly, however, the fact that target-absent decision times in Exp. 3 were subject to the same population variability, with some participants making reliably faster decisions about absence when more of the display was occluded, suggest that incomplete meta-perceptual knowledge cannot be the full explanation. Some participants failed to use beliefs about counterfactual visibility even when it was directly presented to them.

## Future directions

Under our model's assumption, the optimal decision policy is to accumulate evidence (in the form of LLR units, derived from sensory observations) until reaching a fixed boundary. This constitutes an integration-to-bound model, with strong similarity to the commonly used Drift Diffusion Model [DDM, @ratcliff2008; @shadlen2013]. Unlike the DDM, however, evidence accumulation in our model does not follow a normal distribution, but an asymmetric categorical distribution. Despite this difference, it may be possible to derive an approximate mapping from DDM parameters (such as the drift rate, boundary separation, and starting point bias) to parameters in our model (visibility, expected visibility, and temporal discounting rate). Such a mapping has been proposed, for example, between DDM parameters to the parameters of a symmetrical Bayesian agent with a pre-defined certainty threshold [@bitzer2014]. A mapping between POMDP and DDM parameters may allow researcher to fit ideal observer models to observed data, via a process-model surrogate.

Second, we see this work as establishing that many—but not all—individuals use beliefs about expected or counterfactual visibility to inform their perceptual decisions, and that traces of these beliefs can be identified in the way detection decisions are made in the absence of a target. For our purpose here, we used the most obvious visibility manipulation that we could think of: partial stimulus occlusion. Future research can use target-absent trials to infer what people know and believe about their own perception. For example, it may turn out that some manipulations affect visibility without affecting expected visibility, or vice versa: affecting expected visibility with no real effect on visibility\^[In fact, we started this line of work after some of us (MM and CP, in prep.) identified a manipulation which affects visibility without affecting target-absent trials.]. This way, inferences in the absence of a target can provide a window into implicit metacognitive knowledge about perception [@mazor2021inference].

Finally, the robust individual differences we reveal here raise fundamental questions for future research. Do variants $A$ and $B$ differ in other aspects of their perception and cognition, apart from the ones we observe here? Specifically, do they also vary in their tendency to consider counterfactuals outside perception [for example, in making inferences based on vignettes, @byrne1999] ? Do they vary in how prior beliefs affect their perception more broadly[for example, in their susceptability to cue-stimulus conditioning, @powers2017; @kok2013; @press2020]? And, are those participants whose behaviour is more aligned with variant $B$ simply less rational, or alternatively, can their disregard of counterfactual visibility be rational under some assumptions, or especially adapted to some environmental settings? We are eager to find out the answer to these questions as future research will elucidate the theoretical significance of these individual differences.

## Conclusion

Overall, analysis of decision criteria, reaction times and decision confidence, followed by a more focused examination of individual differences, indicate that some—but not all—humans take into account beliefs about counterfactual visibility when making perceptual detection judgments, and when rating their subjective confidence in such decisions. We do not perceive absence when absence is visible, but when presence would have been.
