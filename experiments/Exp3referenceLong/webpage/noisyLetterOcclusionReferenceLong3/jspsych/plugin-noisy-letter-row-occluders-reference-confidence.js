var jsNoisyLetter = (function (jspsych) {
  ...
  const info = {
      ...
      show_trial_number: {
        tpye:jspsych.ParameterType.BOOL,
        default: false
      }
		}
	}
  ...
  class NoisyLetterPlugin {
    ...
    trial(display_element, trial) {
      ...
      trial.max_p = Array.isArray(trial.max_p) ? trial.max_p : [trial.max_p];
      trial.num_stimuli = trial.max_p.length
      ...
      let sketch = (p) => {
        ...
        const p_function = function(frame_number, stim_number) {
          if (!trial.present & stim_number==Math.floor(trial.num_stimuli/2)) {
            return 0
          } else if (trial.flat) {
            return (trial.max_p[stim_number])
          } else {
            return (trial.max_p[stim_number])/(1+Math.exp(-trial.steepness*(frame_number-trial.latency)))
          }
        }
        ...
        var draw_choices = (response) => {
          ...
          if (trial.show_trial_number) {
            p.push()
            p.textSize(15)
            p.fill(0);
            p.translate(window.innerWidth/4,window.innerHeight/4)
            p.text(`${window.trial_number}\\${window.trials_per_block}`,0,0)
            p.pop()
          }
      ...
      var rate_confidence = (confidence) => {
        ...
        p.setup = () => {
          ...
          trial.stim_offset = this.img.width*trial.pixel_size_factor*2
        ...
        p.draw = () => {
          ...
          if (p.millis()-window.start_time < trial.RT + trial.post_click_delay) {
            ...
            var presented_frame = [];
            for (let stim_number=0; stim_number<trial.num_stimuli; stim_number++) {
              var presented_stim = [];
              for (let y = 0; y < this.img.height; y++) {
                var presented_row = [];
                for (let x = 0; x < this.img.width; x++) {
                  ...
                  if (window.hidden_rows.includes(y)) {
                    ...
                    if (Math.random() <= p_function(window.frame_number,stim_number)) {
                  ...
                  p.translate(p.width/2+ trial.stim_offset*stim_number +x*trial.pixel_size_factor - this.img.width/2*trial.pixel_size_factor -trial.stim_offset*(trial.num_stimuli-1)/2,
                ...
                presented_stim.push(presented_row);
              }
              presented_frame.push(presented_stim);
            }
          ...
          if ((key=='g' | key=='f') & trial.RT==Infinity) {
            // only regard relevant key presses during the response phase
              ...
              window.trial_data = {
                ...
                hidden_rows: window.hidden_rows
              };
            }
           // save the image
            // if (key === 'q') {
