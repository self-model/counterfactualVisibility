...
```{r split-rt-by-parity, echo=FALSE}

splitRTbyParity <- function(df, iv, iv_levels) {
  ...
  filtered_df <- df %>%
    filter(test_part %in% c('test1','test2') &
             ...
             correct) %>%
    group_by(subj_id) %>%
    mutate(parity = ifelse(seq_along(RT)%%2==0,'odd','even'))
  
  summary_df <- filtered_df %>% 
    group_by(subj_id,parity,present,.data[[iv]]) %>%
...
occlusion.split.df <- splitRTbyParity(occlusion.df,'hide_proportion',c('low','high'))
occlusion.split.summary.df <- occlusion.split.df %>% 
  merge(occlusion.df %>%
          group_by(subj_id) %>%
          summarise(exp=exp[1]))%>%
  group_by(parity,present,other_sign,exp) %>%
...
occlusion.split.summary.df %>%
  ...
  geom_abline(slope=0) +
  facet_wrap(~parity*exp)

occlusion.split.df %>% 
  merge(occlusion.df %>%
          group_by(subj_id) %>%
          summarise(exp=exp[1]))%>%
  dplyr::select(subj_id,present,diff,parity,exp) %>%
...
E2a.split.df %>% 
  dplyr::select(subj_id,present,diff,parity) %>%
  spread(parity,diff) %>%
  ggplot(aes(x=odd,y=even)) +
  geom_point() +
  facet_wrap(~present) +
...
E3a.split.df <- splitRTbyParity(E3a.df %>%
