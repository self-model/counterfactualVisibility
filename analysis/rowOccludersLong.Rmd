...
E2a.overall_descriptives <- E2a.df %>%
    filter(test_part=='test1' | test_part=='test2') %>%
    group_by(subj_id) %>%
...
`r E2a.raw_df%>%pull(subj_id)%>%unique()%>%length()` participants completed the task. Mean accuracy in the main block experiment was `r E2a.overall_descriptives%>%pull(accuracy)%>%mean()%>%printnum()` (SD=`r E2a.overall_descriptives%>%pull(accuracy)%>%sd()%>%printnum()`). The mean median response time was `r printnum(E2a.overall_descriptives%>%pull(RT)%>%mean()/1000)` seconds (SD=`r printnum(E2a.overall_descriptives%>%pull(RT)%>%sd()/1000)`). `r E2a.low_accuracy%>%length()` participants were excluded based on our pre-registered accuracy lower bound of 50%, `r E2a.too_fast%>%length()` were excluded based on our pre-registered criterion for making hasty responses (RT\<100 ms in more than 25% of the trials), and `r E2a.too_slow%>%length()` participants were excluding for having response times longer than 7 seconds in more than 25% of the trials, leaving `r E2a.df$subj_id%>%unique()%>%length()` for the main analysis. 


```{r H1, echo=FALSE, cache=TRUE}

E2a.RT_by_resp <- E2a.df %>%
...
*Hypothesis 1 (PRESENCE/ABSENCE RESPONSE TIME)*: As expected, response times were significantly shorter in decisions about presence compared to absence (`r printnum(E2a.RT_by_resp%>%pull(respTRUE)%>%mean()/1000)` vs `r printnum(E2a.RT_by_resp%>%pull(respFALSE)%>%mean()/1000)` seconds; `r apa_print(E2a.RT_by_resp%>%pull(diff)%>%t.test())$statistic`).


```{r H2, echo=FALSE, cache=TRUE}

E2a.RT_by_occlusion_in_presence <- E2a.df %>%
...
E2a.RT_by_occlusion_in_presence_correct_only <- E2a.df %>%
...
*Hypothesis 2 (OCCLUSION RT EFFECT IN PRESENCE)*: Target-present decisions were significantly slower when more of the target was occluded behind black rows (`r printnum(E2a.RT_by_occlusion_in_presence_correct_only$hide_proportion0.35%>%mean()/1000)` vs `r printnum(E2a.RT_by_occlusion_in_presence_correct_only$hide_proportion0.1%>%mean()/1000)` seconds;  `r apa_print(E2a.RT_by_occlusion_in_presence_correct_only%>%pull(diff)%>%t.test())$statistic`).

```{r H3, echo=FALSE, cache=TRUE}

E2a.RT_by_occlusion_in_absence <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2') & RT>100 & RT<5000  &!resp) %>%
  group_by(subj_id,hide_proportion) %>%
  summarise(RT=median(RT))%>%
  spread(hide_proportion,RT,sep='')%>%
...
E2a.RT_by_occlusion_in_absence_correct_only <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2') & RT>100 & RT<5000  &!resp & correct) %>%
  group_by(subj_id,hide_proportion) %>%
  summarise(RT=median(RT))%>%
  spread(hide_proportion,RT,sep='')%>%
...
*Hypothesis 3 (OCCLUSION RT EFFECT IN ABSENCE)*: Target-absent decision times were unaffected by the occlusion manipulation (`r printnum(E2a.RT_by_occlusion_in_absence_correct_only$hide_proportion0.35%>%mean()/1000)` vs `r printnum(E2a.RT_by_occlusion_in_absence_correct_only$hide_proportion0.1%>%mean()/1000)` seconds for the 2 and 6 rows conditions, respectively;  `r apa_print(E2a.RT_by_occlusion_in_absence_correct_only%>%pull(diff)%>%t.test())$statistic`).


```{r H4, echo=FALSE, cache=TRUE}

E2a.RT_by_occlusion_and_response <- merge(
  E2a.RT_by_occlusion_in_presence,
  E2a.RT_by_occlusion_in_absence,
  by= 'subj_id',
  suffixes = c('presence','absence')) %>%
  mutate(interaction = diffpresence-diffabsence);

E2a.RT_by_occlusion_and_response_correct_only <- merge(
  E2a.RT_by_occlusion_in_presence_correct_only,
  E2a.RT_by_occlusion_in_absence_correct_only,
  by= 'subj_id',
  suffixes = c('presence','absence')) %>%
  mutate(interaction = diffpresence-diffabsence);
```

*Hypothesis 4 (OCCLUSION RESPONSE INTERACTION ON RT)*: The effect of occlusion on response time was stronger in decisions about target presence, compared to decisions about target absence (`r apa_print(E2a.RT_by_occlusion_and_response_correct_only%>%pull(interaction)%>%t.test())$statistic`).
...
E2a.quantile_occlusion_effects <- E2a.quantiles %>%
...
E2a.quantile_summary <- E2a.quantiles %>% 
  group_by(subj_id) %>%
...
E2a.quantile_plot <- E2a.quantile_summary %>% 
...
ggsave('figures/E2aQuantiles_separate.png',E2a.quantile_plot,width=3.5,height=3.5);
...
E2a.descriptives_by_occlusion<- E2a.df %>%
  filter(test_part=='test1' | test_part=='test2') %>%
...
E2a.dprime <- E2a.descriptives_by_occlusion%>%
  dplyr::select(subj_id,hide_proportion,d)%>%
  spread(hide_proportion, d,sep='')%>%
  mutate(diff=hide_proportion0.1-hide_proportion0.35)

E2a.criterion <- E2a.descriptives_by_occlusion%>%
  dplyr::select(subj_id,hide_proportion,c)%>%
  spread(hide_proportion, c,sep='')%>%
  mutate(diff=hide_proportion0.1-hide_proportion0.35)

E2a.hit_rate <- E2a.descriptives_by_occlusion%>%
  dplyr::select(subj_id,hide_proportion,hit_rate)%>%
  spread(hide_proportion, hit_rate,sep='')%>%
  mutate(diff=hide_proportion0.1-hide_proportion0.35)

E2a.fa_rate <- E2a.descriptives_by_occlusion%>%
  dplyr::select(subj_id,hide_proportion,fa_rate)%>%
  spread(hide_proportion, fa_rate,sep='')%>%
  mutate(diff=hide_proportion0.1-hide_proportion0.35)
```

*Hypothesis 5 (SENSITIVITY)*: Signal detection sensitivity was significantly impacted by pixel occlusion (`r apa_print(E2a.dprime$diff%>%t.test())$statistic`), with d'=`r printnum(E2a.dprime$hide_proportion0.1%>%mean())` (SD=`r printnum(E2a.dprime$hide_proportion0.1%>%sd())`) when two rows were occluded, and d'=`r printnum(E2a.dprime$hide_proportion0.35%>%mean())` (SD=`r printnum(E2a.dprime$hide_proportion0.35%>%sd())`) when six rows were occluded. This effect was driven by a decrease in hit rate (`r apa_print(E2a.hit_rate$diff%>%t.test())$statistic`), and a smaller decrease in the false-positive rate (`r apa_print(E2a.fa_rate$diff%>%t.test())$statistic`) when six rows were occluded.

*Hypothesis 6 (CRITERION)*: Signal detection criterion was conservative overall, and more conservative when more of the display was occluded (`r apa_print(E2a.criterion$diff%>%t.test())$statistic`), with c=`r printnum(E2a.criterion$hide_proportion0.1%>%mean())` (SD=`r printnum(E2a.criterion$hide_proportion0.1%>%sd())`) when two rows were occluded, and `r printnum(E2a.criterion$hide_proportion0.35%>%mean())` (SD=`r printnum(E2a.criterion$hide_proportion0.35%>%sd())`) when six rows were occluded. 
...
ggsave('figures/E2aRT.png',E2a.rt_plot,width=3.5,height=3.5);

```

```{r participant-specific-plot-separate, echo=FALSE, cache=TRUE }
...
ggsave('figures/E2aRT_present.png',E2a.rt_present_plot,width=2.2,height=3.5);
...
ggsave('figures/E2aRT_absent.png',E2a.rt_absent_plot,width=2.2,height=3.5);
```
```{r sign consistency analysis of present responses}

E2a.hit.directional <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2')) %>%
  filter(present==1) %>%
...
E2a.hit.sign_consistency <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2')) %>%
  filter(present==1) %>%
  dplyr::select(subj_id,hide_proportion,resp) %>%
  drop_na()%>%
...
E2a.fa.sign_consistency <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2')) %>%
  filter(present==0) %>%
  dplyr::select(subj_id,hide_proportion,resp) %>%
  drop_na()%>%
...
E2a.present_summary <- E2a.df %>%
  filter((test_part=='test1' | test_part=='test2') & RT>100 & RT<5000) %>%
  group_by(subj_id,hide_proportion,present) %>%
  summarise(ppresent = mean(resp)) 
  
E2a.present_sc <- E2a.hit.sign_consistency$consistency_per_id %>%
  mutate(present=1) %>%
  rbind(E2a.fa.sign_consistency$consistency_per_id %>%
          mutate(present=0))

E2a.present_summary <- E2a.present_summary %>%
  merge(E2a.present_sc, by=c('subj_id','present')) %>%
  mutate(present = factor(present, levels=c(1,0), labels=c('present','absent')),
         occluded_rows = ifelse(hide_proportion==0.1,2,6))

min_sc = E2a.present_sc$score%>%min()
max_sc = E2a.present_sc$score%>%max()

E2a.present_plot<- E2a.present_summary %>% 
 ggplot(aes(x=occluded_rows, 
             y=ppresent, 
             color=present,
             group=interaction(subj_id,present),
             alpha=score)) +
  geom_line(data=E2a.present_summary,aes(x=as.numeric(occluded_rows)-0.6+0.4*as.numeric(present)),size=1.5)+
  geom_point(data=E2a.present_summary,aes(x=as.numeric(occluded_rows)-0.6+0.4*as.numeric(present)),size=3)+
  # geom_label(data= E2a.RT %>%
  #              filter(subj_id %in% sig_subs),
  #            fill='white',alpha=1)+
  scale_color_manual(values=c("#377eb8", "#E41a1c"))+
  ...
  ggsave(paste('figures/E2a_present_separate.png',sep=''),E2a.present_plot,width=3.5,height=2);

```

```{r correlations}

E2a.correlation_matrix <- cor(
  E2a.df %>%
  filter((test_part=='test1' | test_part=='test2') & RT>100 & RT<5000) %>%
  group_by(subj_id) %>%
...
plot_sign_consistency(E2a.ta.sign_consistency,"#e41a1c",'figures/E2a_SC_RT_ta.png')
plot_sign_consistency(E2a.tp.sign_consistency,"#377eb8",'figures/E2a_SC_RT_tp.png')

plot_sign_consistency(E2a.hit.sign_consistency,"#e41a1c",'figures/E2a_SC_hit.png')
plot_sign_consistency(E2a.fa.sign_consistency,"#e41a1c",'figures/E2a_SC_fa.png')
...
E2.random_subset_df %>%
  ...
  theme_bw() +
  theme(legend.pos='na', text = element_text(size = 14)) +
  geom_vline(data = E4.random_subset_df %>% filter(sampled==1),
             aes(xintercept=effect))

ggsave('figures/E2a_subjects_histogram.png', width = 6, height = 2, units = "in", dpi = 300)
